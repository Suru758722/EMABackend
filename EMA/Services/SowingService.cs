...
namespace EMA.Services
{
    ...
    {
        dynamic GetData(int takes);
        bool AddUpdate(SowingModel data);
        bool Delete(int Id);
    }
    public class SowingService : ISowingService
    {
        readonly ApplicationDbContext _context;
        public SowingService(ApplicationDbContext context)
        {
            _context = context;
        }
        ...
        {
            var list =  _context.Sowing.Include(x => x.Machine).Include(x => x.Equipment).Include(x => x.FarmerDetail).ThenInclude(x => x.Crop).AsQueryable();
        ...
        public bool AddUpdate(SowingModel data)
        {
            ...
            Sowing _doesExist = _context.Sowing.Where(x => x.Id == data.Id).FirstOrDefault();
            if (_doesExist == null)
            {

                Sowing _detail = new Sowing();
        ...
        {
            Sowing _doesExist = _context.Sowing.Where(x => x.Id == Id).FirstOrDefault();

            _context.Sowing.Remove(_doesExist);
            _context.SaveChanges();
            return true;
        }
    }
