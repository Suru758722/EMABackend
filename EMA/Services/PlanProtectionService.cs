...
namespace EMA.Services
{
    public interface IPlantProtectionService
    {
        ...
        bool AddUpdate(PlantProtectionModel data);
        bool Delete(int Id);
    }
    public class PlantProtectionService : IPlantProtectionService
    {
        readonly ApplicationDbContext _context;
        public PlantProtectionService(ApplicationDbContext context)
        {
            _context = context;
        }
        ...
        {
            var list =  _context.PlantProtection.Include(x => x.Machine).Include(x => x.Equipment).Include(x => x.FarmerDetail).ThenInclude(x => x.Crop).AsQueryable();
        ...
        public bool AddUpdate(PlantProtectionModel data)
        {
            ...
            PlantProtection _doesExist = _context.PlantProtection.Where(x => x.Id == data.Id).FirstOrDefault();
            if (_doesExist == null)
            {

                PlantProtection _detail = new PlantProtection();
                ...
                _context.PlantProtection.Add(_detail);
        ...
        {
            PlantProtection _doesExist = _context.PlantProtection.Where(x => x.Id == Id).FirstOrDefault();

            _context.PlantProtection.Remove(_doesExist);
            _context.SaveChanges();
            return true;
        }
    }
